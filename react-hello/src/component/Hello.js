...
const Hello = () => {
  /* 
    ...
    변수를 만들 때 useState() 함수를 사용하여 만든다
    useState() 함수는 state 변수와 setState() 함수를 생성한다
    state 변수를 화면에 표현하고자 하는 곳에 위치시켜두면
    react가 state 변수의 상태를 항상 감시한다
    어딘가에서  steState() 함수를 통하여 state 변수값이 변화되면 
    화면에 표시된 모든 state 변수를 다시 그린다

    react 에서는 디자인 단계에서 변수가 표시될 위치를 지정하면 그 이후에 변화된 값을 다시 지정하여 그려라 라는 코드를 사용하지 않는다

    */
  const [counter, setCounter] = useState(0);
  const [stName, steStName] = useState("홍길동");
